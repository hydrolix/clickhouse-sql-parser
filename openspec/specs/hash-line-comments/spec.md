...
Recognise `#` as a single-line comment terminator in the ClickHouse SQL lexer, with the same scoping rules as `--`, so that real-world queries — particularly those generated by dashboard tools that mix shell-style and SQL-style commentary — lex successfully and reach the parser. This is a lexer-only extension that does not touch the parser, the AST, the formatter, or any existing golden test fixture.
