...
Recognise `SELECT … EXCEPT col …` as a bare-identifier shorthand for the single-column case of `SELECT … EXCEPT (col) …` in the ClickHouse SQL parser. ClickHouse SQL accepts two shapes for the `EXCEPT` modifier on a SELECT item: the parenthesised list `SELECT * EXCEPT (col1, col2) FROM t` and the bare-identifier form `SELECT * EXCEPT col FROM t` when there is exactly one column to exclude. The bare-ident shorthand is common in dashboard- and tool-generated SQL because it reads naturally and matches how `EXCEPT` is documented for the single-column case. Today the parser rejects the bare-ident form with `"expected the last token kind is: (, but got <ident>"` — the `EXCEPT`/`APPLY`/`REPLACE` arm of `parseSelectItem` unconditionally dispatches to `parseFunctionExpr`, which immediately requires `(`. This change adds the shorthand as a peer of the parenthesised form, producing the same AST shape so downstream consumers (formatter, visitor, JSON marshaller) do not need to distinguish the two surface syntaxes.
